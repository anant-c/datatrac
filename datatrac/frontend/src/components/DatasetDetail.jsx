import { Calendar, Download, HardDrive, Hash, Info, Trash2, Upload } from 'lucide-react';
import PropTypes from 'prop-types';
import { useEffect, useState } from 'react';
import { API_BASE_URL, formatDateTime, formatSize } from '../lib/utils';
import { DownloadModal } from './DownloadModal';
import { LineageDisplay } from './LineageDisplay';

// --- CORRECTED PART 1 ---
// Added `onDatasetDeleted` to the list of props being received.
export function DatasetDetail({ dataset, onDatasetUpdate, onDatasetDeleted, onSelectDataset }) {
    ...
    useEffect(() => {
        if (dataset) {
            const fetchLineage = async () => {
                ...
                setLineage({ parents: [], children: [] }); // Reset lineage
                try {
                    const response = await fetch(`${API_BASE_URL}/datasets/${dataset.hash}/lineage`);
...
DatasetDetail.propTypes = {
    ...
    onDatasetDeleted: PropTypes.func.isRequired,
